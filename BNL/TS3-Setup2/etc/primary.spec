...
TEMP_SET    Float   ${ts2/ccdtempsetpoint}       Temperature set point (deg C)
CCDTEMP     Float   ${ts2/ccdtemperature}        Measured temperature (deg C)
CCDBSS      Float   ${ts2/ccdbiasvoltage}        CCD bias voltage
MONDIODE    Float   ${ts2/photodiodecurrent}     Current in the monitoring diode (nA)
MONOWL      Float   ${MonochromatorWavelength}  Monochromator wavelength (nm)
PIXRATE     Float   ${PixelReadRate}            Rate for pixel reads
FILTER      String  ${ts2/TSState/filter}        Name of the filter
FILTPOS     Integer ${ts2/filter1position}       Filter position
...
CCDGAIN     Float   4.00                        Rough guess at overall system gain (e-/DNB)
...
HIERARCH.MONOCH-WAVELENG Float   ${ts2/monochromatorwavelength} monochromator wavelength
HIERARCH.MONOCH-SLIT_A   Float   ${ts2/monochromatorslit1}     Width of the A slit in um
HIERARCH.MONOCH-SLIT_B   Float   ${ts2/monochromatorslit2}     Width of the B slit in um
HIERARCH.MONOCH-SLIT_C   Float   0                            Width of the C slit in um
HIERARCH.MONOCH-BANDPASS Float   ${ts2/monochromband}          Automatic slit width in nm
HIERARCH.MONOCH-FILT_1   Float   ${ts2/filter1position}        filter position
HIERARCH.MONOCH-MSTEPS   Float   ${ts2/monochromstep}          Current grating position in terms of motor step
HIERARCH.MONOCH-GRATING  Float   ${ts2/monochromgrating}       Grating position
HIERARCH.AMP0-IDN        String  ${ts2/TSState/pdtype}         Monitor Photodiode model number
HIERARCH.AMP0-AZERO      String  F                            SYSTEM:AZERO value
HIERARCH.AMP0-COUNT      Integer ${ts2/TSState/pdcnt}          number of measurements (buffer length)
HIERARCH.AMP0-MEAS_NPLC  Integer ${ts2/TSState/pdnplc}         Time of each measurment. In Hz multiples (1 = 1
HIERARCH.AMP2-IDN        String  ${ts2/TSState/biastype}       identification string
HIERARCH.AMP2-ON         Integer ${ts2/TSState/biasstate}      If voltage source is switched on
HIERARCH.AMP2-VOLTAGE    Float   ${ts2/ccdbiasvoltage}         [V] voltage level
HIERARCH.AMP2-CURRENT    Float   ${ts2/TSState/picocurrent}    [pA] measured current (in picoAmps)
